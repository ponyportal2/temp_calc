...
//----------------------------------------------------------------------------------
RAYGUIAPI void GuiTextBoxSetActive(
    Rectangle bounds);                          // Sets the active textbox
RAYGUIAPI Rectangle GuiTextBoxGetActive(void);  // Get bounds of active textbox

RAYGUIAPI void GuiTextBoxSetCursor(
    int cursor);  // Set cursor position of active textbox
RAYGUIAPI int GuiTextBoxGetCursor(
    void);  // Get cursor position of active textbox

RAYGUIAPI void GuiTextBoxSetSelection(
    int start, int length);  // Set selection of active textbox
RAYGUIAPI Vector2
GuiTextBoxGetSelection(void);  // Get selection of active textbox (x - selection
                               // start  y - selection length)

RAYGUIAPI bool GuiTextBoxIsActive(
    Rectangle bounds);  // Returns true if a textbox control with specified
                        // `bounds` is the active textbox
RAYGUIAPI GuiTextBoxState
GuiTextBoxGetState(void);  // Get state for the active textbox
RAYGUIAPI void GuiTextBoxSetState(
    GuiTextBoxState state);  // Set state for the active textbox (state must be
                             // valid else things will break)

RAYGUIAPI void GuiTextBoxSelectAll(
    const char *text);  // Select all characters in the active textbox (same as
                        // pressing `CTRL` + `A`)
RAYGUIAPI void GuiTextBoxCopy(
    const char *text);  // Copy selected text to clipboard from the active
                        // textbox (same as pressing `CTRL` + `C`)
RAYGUIAPI void GuiTextBoxPaste(
    char *text, int textSize);  // Paste text from clipboard into the textbox
                                // (same as pressing `CTRL` + `V`)
RAYGUIAPI void GuiTextBoxCut(
    char *text);  // Cut selected text in the active textbox and copy it to
                  // clipboard (same as pressing `CTRL` + `X`)
RAYGUIAPI int GuiTextBoxDelete(
    char *text, int length,
    bool before);  // Deletes a character or selection before from the active
                   // textbox (depending on `before`). Returns bytes deleted.
RAYGUIAPI int GuiTextBoxGetByteIndex(
...
RAYGUIAPI bool GuiTextBoxEx(Rectangle bounds, char *text, int textSize,
                            bool editMode);

RAYGUIAPI static void DrawTextRec(
...
RAYGUIAPI static void DrawTextRecEx(
    Font font, const char *text, Rectangle rec, float fontSize, float spacing,
...
RAYGUIAPI static void DrawTextBoxedSelectable(
    Font font, const char *text, Rectangle rec, float fontSize, float spacing,
    bool wordWrap, Color tint, int selectStart, int selectLength,
    Color selectTint, Color selectBackTint);  // Alias for above
...
static void DrawTextRec(Font font, const char *text, Rectangle rec,
                        ...
                        Color tint) {
  DrawTextRecEx(font, text, rec, fontSize, spacing, wordWrap, tint, 0, 0, WHITE,
                WHITE);
}

// Draw text using font inside rectangle limits with support for text selection
static void DrawTextRecEx(Font font, const char *text, Rectangle rec,
                          float fontSize, float spacing, bool wordWrap,
                          Color tint, int selectStart, int selectLength,
                          Color selectTint, Color selectBackTint) {
  int length = TextLength(text);  // Total length in bytes of the text, scanned
                                  // by codepoints in loop
...
static void DrawTextBoxedSelectable(Font font, const char *text, Rectangle rec,
  ...
  DrawTextRecEx(font, text, rec, fontSize, spacing, wordWrap, tint, selectStart,
...
//----------------------------------------------------------------------------------
static Rectangle guiTextBoxActive = {
    0};  // Area of the currently active textbox
...
static int GuiMeasureTextBoxRev(
    ...
    int *pos);  // Highly synchronized with calculations in
                // DrawTextBoxedSelectable()

static inline int GuiTextBoxGetCursorCoordinates(
    const char *text, int length, Rectangle rec,
    int pos);  // Calculate cursor coordinates based on the cursor position
               // `pos` inside the `text`.
static inline int GuiTextBoxGetCursorFromMouse(
    const char *text, int length, Rectangle rec,
    int *pos);  // Calculate cursor position in textbox based on mouse
                // coordinates.
static inline int GuiTextBoxMaxCharacters(
    const char *text, int length,
    Rectangle rec);  // Calculates how many characters is the textbox able to
                     // draw inside rec
...
RAYGUIAPI Vector2 GuiTextBoxGetSelection(void) {
  if (guiTextBoxState.select == -1 ||
    ...
    return RAYGUI_CLITERAL(Vector2){
        (float)guiTextBoxState.select,
        (float)guiTextBoxState.cursor - guiTextBoxState.select};

  return RAYGUI_CLITERAL(Vector2){
      (float)guiTextBoxState.cursor,
      (float)guiTextBoxState.select - guiTextBoxState.cursor};
}
...
RAYGUIAPI int GuiTextBoxGetByteIndex(const char *text, int start, int from,
                                     int to) {
  int i = start, k = from;
  ...
  if ((guiTextBoxState.cursor != -1) && (text != NULL)) {
    ...
    if ((guiTextBoxState.select != -1) &&
      ...
      guiTextBoxState.cursor =
          start;  // Always set cursor to start of selection
...
#define TEXTBOX_CURSOR_COOLDOWN 5
  ...
  if ((state != STATE_DISABLED) && !guiLocked) {
    ...
    if (editMode) {
      ...
      if (active) {
        ...
        // We aren't drawing all of the text so make sure
        // `DrawTextBoxedSelectable()` is selecting things correctly
  ...
  if (state == STATE_PRESSED) {
    ...
    if (editMode && active && ((framesCounter / 30) % 2 == 0) &&
        selLength == 0) {
      DrawRectangle(
          cursorPos.x, cursorPos.y - 1, 1, GuiGetStyle(DEFAULT, TEXT_SIZE) + 2,
  ...
  DrawTextRecEx(
      ...
      selStart, selLength, GetColor(GuiGetStyle(TEXTBOX, TEXT_COLOR_FOCUSED)),
      GetColor(GuiGetStyle(TEXTBOX, BASE_COLOR_FOCUSED)));
  ...
  for (i = 0; i < length; i++, k++) {
    ...
    if (letter != '\n') {
      ...
      if ((mode == GUI_MEASURE_MODE_CURSOR_POS) && (*pos == k))
        ...
        Rectangle grec = {
            rec.x + textOffsetX - 1, rec.y, (float)glyphWidth,
            (font.baseSize + font.baseSize / 2) * scaleFactor - 1};
